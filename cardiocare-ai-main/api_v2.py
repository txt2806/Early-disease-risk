...
_DEFAULT_MODEL_V2_PATH = Path(__file__).resolve().parent / "heart_model_tuned_v2.pkl"
...
def health():
    return {
        ...
        "model_v2" : "RandomForest Multi-class (BalAcc)" if model_v2 else "not loaded",
        "metrics_v1": metrics,
...
@app.get("/model/info")
def model_info():
    """Thông tin chi tiết về model v1 và v2"""
    ensure_model_ready()
    return {
        "v1": {
        ...
        "v2": {
            "algorithm"       : "Random Forest Multi-class 4 lớp (D.14)",
            "dataset"         : "UCI Heart Disease 920 patients (oversample)",
            "balanced_accuracy": metrics_v2.get("balanced_accuracy", "N/A"),
            "macro_f1"        : metrics_v2.get("macro_f1", "N/A"),
            "n_classes"       : 4,
...
PATIENT_CATEGORICAL_DEFAULTS = {
    "fbs"    : "FALSE",            # không biết tiền sử đường huyết cao -> giả định bình thường
    "exang"  : "FALSE",            # không biết đau ngực khi gắng sức -> giả định không có
...
DOCTOR_SYSTEM_PROMPT = """Bạn là trợ lý AI hỗ trợ BÁC SĨ trong hệ thống CardioCare —
một nền tảng phát hiện sớm bệnh tim mạch dùng Random Forest + SHAP.
...
- Khi được hỏi về một kết quả dự đoán cụ thể (predict_context nếu có),
  ...
  (ví dụ: model chỉ đạt balanced_accuracy ~0.48 cho bài toán 4 lớp do
  dataset mất cân bằng — bác sĩ cần biết giới hạn này).
...
def _build_user_message_with_context(message: str, predict_context: Optional[dict]) -> str:
    """Nếu có predict_context, ghép vào đầu tin nhắn để model có thông tin."""
    if not predict_context:
        return message
    return (
        f"[Bối cảnh — kết quả dự đoán AI gần nhất]: {predict_context}\n\n"
