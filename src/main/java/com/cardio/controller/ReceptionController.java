...
public class ReceptionController {
    ...
    private StaffProfile getCurrentStaff(UserDetails userDetails) {
        return staffRepository.findByUsernameIgnoreCase(userDetails.getUsername())
                .orElseThrow(() -> new RuntimeException("Staff profile not found for user: " + userDetails.getUsername()));
    }
    ...
    public String assignAppointment(@AuthenticationPrincipal UserDetails userDetails,
        ...
        try {
            ...
            if (doctorId != null) {
                ...
                // Exclude the current appointment from the count if it's already assigned to this doctor and date
            ...
            sysLog.setDetails("Lễ tân " + staff.getFullName() + " đã cập nhật lịch hẹn #" + appointmentId + 
                              ": Bác sĩ: " + (appointment.getDoctor() != null ? appointment.getDoctor().getFullName() : "Chưa phân công") + 
    ...
    public String saveAppointment(@AuthenticationPrincipal UserDetails userDetails,
        ...
        try {
            ...
            sysLog.setDetails("Lễ tân " + staff.getFullName() + " tạo lịch hẹn khám mới cho bệnh nhân: " + patient.getFullName() + 
                              " (BS: " + (appointment.getDoctor() != null ? appointment.getDoctor().getFullName() : "Chưa phân công") + 
                              ", Phòng: " + (roomNumber != null ? roomNumber : "Chưa xếp") + 
                              ", Trạng thái: " + status + ")");
            sysLog.setTimestamp(LocalDateTime.now());
            systemLogRepository.save(sysLog);
